...
void branch_and_bound_with_extended_list(char start, char goal) {
    typedef struct {
        int cost;
        char node;
        char path[MAX_NODES];
    ...
    queue[queue_size++] = (Node){0, start, {start, '\0'}};

    int best_cost = INF;
    char best_path[MAX_NODES];

    int extended_list[MAX_NODES] = {INF}; // Initialize extended list to infinity

    while (queue_size > 0) {
        // Find the node with the lowest cost
        int best_index = 0;
        for (int i = 1; i < queue_size; i++) {
            if (queue[i].cost < queue[best_index].cost) {
                best_index = i;
            }
        }
        ...
        if (current_node.node == goal && current_node.cost < best_cost) {
            best_cost = current_node.cost;
            strcpy(best_path, current_node.path);
        }

        // Check if the node is already in the extended list with a lower cost
        int current_index = current_node.node - 'A';
        if (extended_list[current_index] <= current_node.cost) {
            continue;
        }
        extended_list[current_index] = current_node.cost;

        // Expand neighbors
        for (int j = 0; j < edge_count[current_index]; j++) {
            char neighbor = graph[current_index][j].node;
            int edge_cost = graph[current_index][j].cost;
            int total_cost = current_node.cost + edge_cost;
    ...
    if (best_cost < INF) {
        printf("Best Path Found (Branch and Bound with Extended List): %s\n", best_path);
...
int main() {
    ...
    // Run Branch and Bound with Extended List
    branch_and_bound_with_extended_list(start_node, goal_node);
